...
const Navbar: React.FC = () => {
  ...
  return (
    <nav
      className={`${
        styles.paddingX
      } w-full flex items-center py-5 fixed top-0 z-20 px-8 md:px-0 ${
        scrolled
      ...
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          ...
          <div
            className={`${
              !toggle ? "hidden" : "flex"
            } p-6 indigo-gradient absolute top-20 right-0 mx-4 my-2 min-w-[170px] z-10 rounded-xl`}
          >
              ...
              {navLinks.map((nav) => (
                <li
                  ...
                  className={`font-poppins font-medium cursor-pointer text-[16px] ${
                    active === nav.title ? "text-tertiary" : "text-white"
                  }`}
                  onClick={() => {
                    setToggle(!toggle);
                    setActive(nav.title);
